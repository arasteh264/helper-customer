...
import { onboardingSteps } from "../api/mock-data";

const fa = new Intl.NumberFormat("fa-IR", { minimumIntegerDigits: 2 });

export function OnboardingSteps() {
  return (
    <section id="how-it-works" className="scroll-mt-20 bg-foreground/[0.03] py-16 sm:py-20">
      <Container>
        <SectionHeading align="center" eyebrow="مراحل" title="در ۴ قدم شروع کنید" />
        ...
        <ol className="relative mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">
          ...
          {onboardingSteps.map(({ icon: Icon, title, description }, index) => (
            <li key={title} className="relative flex gap-4 sm:flex-col sm:items-center sm:text-center">
              ...
              <div>
                <p className="text-xs font-medium text-primary">{fa.format(index + 1)}</p>
                <h3 className="mt-1 text-base font-semibold text-foreground">{title}</h3>
                <p className="mt-2 text-sm leading-7 text-foreground/60">{description}</p>
              </div>
            </li>
          ))}
        </ol>
